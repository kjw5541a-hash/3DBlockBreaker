...
func _test_swing_accelerates_ball() -> void:
	...
	f.ball_pos = Vector2(0.0, Tuning.PADDLE_BAND_MIN_V + 0.6)
	f.ball_vel = Vector2(0.0, -Tuning.v_min())
	# 손가락을 앞으로 밀면서 받는다. 스프링이 아니라 추종이므로 밴드 아래끝에서
	# 위끝을 목표로 준다.
	for i in 240:
...
func _test_dropping_ball_tracks_the_paddle_sideways() -> void:
	...
	var target_x := 1.5
	for i in 60:
		# 세로는 홈에 둔다. 여기서 볼 것은 x 추종뿐인데 패들을 아래로 끌면
		# 낙하 거리가 밴드 높이만큼 늘어 60 프레임 안에 안 닿는다.
		f.step(Vector2(target_x, Tuning.PADDLE_BAND_MAX_V), DT)
		if f.attached:
			break
...
func _pulled_to(pull_v: float) -> PlayField:
	var f := PlayField.new()
	f.grid.fill_all(0)
	f.paddle.pos.y = pull_v
	f.step(Vector2(0.0, pull_v), DT)
	return f

func _spring_launch_speed(pull_v: float) -> float:
